        ...
        const Row = ({ action, late }) => {
          ...
          return (
            <div onClick={() => ent && navigate(`/entreprises/${ent.id}`)} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-colors ${late ? 'bg-red-50 hover:bg-red-100' : isToday ? 'bg-amber-50 hover:bg-amber-100' : 'bg-gray-50 hover:bg-gray-100'}`}>
              ...
              <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate flex items-center gap-2">{ent?.name || '?'}{ent?.status === 'client' && <span className="badge bg-emerald-50 text-emerald-700 !text-[10px] !py-0">Client</span>}</p><p className="text-xs text-gray-500">{action.next_action} — {performer?.full_name}</p></div>
