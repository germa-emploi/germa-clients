...
export default function ActionsDuJour() {
  ...
  useEffect(() => { (async () => {
    setLoading(true)
    const [p, e, a, i, sug, sc] = await Promise.all([
      ...
      fetchAll('ia_suggestions', { filters: { date: todayISO(), profile_id: profileId }, order: { column: 'rank', ascending: true } }),
      fetchAll('ia_scores'),
    ])
    setProfile(p[0] || null); setEnterprises(e); setActions(a); setInterlocuteurs(i)
    setSuggestions(sug || []); setScores(Object.fromEntries((sc || []).map(x => [x.enterprise_id, x])))
    setLoading(false)
  })() }, [profileId])
          ...
          {suggestions.filter(sg => !sg.kind || sg.kind === 'jour').map(sg => { const e = entById[sg.enterprise_id]; if (!e) return null; return (
            <div key={sg.id} onClick={() => navigate(`/entreprises/${e.id}`)} className="px-4 py-3 rounded-xl bg-violet-50/70 hover:bg-violet-50 cursor-pointer transition-colors">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-right text-xs flex-shrink-0"><p className="text-violet-700 font-medium">{sg.suggested_action}</p>{e.phone && <p className="text-gray-500">{e.phone}</p>}</div>
              </div>
            </div>
          ) })}
        </div>
